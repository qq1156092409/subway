<?php
/**
 * TOP API: taobao.travel.item.modify request
 * 
 * @author auto create
 * @since 1.0, 2016.06.27
 */
class TravelItemModifyRequest
{
	/** 
	...
	/** 
	 * 回程交通，仅限跟团游填写且跟团游必填； 回程交通（1-飞机，2- 高铁或动车， 3-汽车，4-游船，10-其他）
	 **/
	...
	/** 
	 * 支持宝贝信息的删除，各个参数的名称之间用【,】分割, 如果对应的参数有设置过值，即使在这个列表中，也不会被删除; 目前支持此功能的宝贝信息如下：locality_life表示删除电子凭证，merchant表示删除码商信息，network_id表示删除电子凭证网点信息,seller_cids表示删除关联店铺类目，outer_id表示删除商家外部编码，second_kill表示删除秒杀信息，traffic_infos 表示大交通信息 hotel_infos表示酒店信息 ticket_infos表示景点门票信息 visa_desc 表示签证信息 rental_car_desc表示租车信息  dining_desc 表示餐饮信息 insurance_desc表示保险信息 other_cost_desc表示其他费用信息 itinerarys 表示行程描述
	 **/
	private $emptyFields;
	...
	/** 
	 * 出发地（城市），填写中文，必须对应类目下存在的出发地，出发地仅限一个;如果提示出发地不存在，请通过http://bangpai.taobao.com/group/thread/14051111-280945273.htm?spm=181.7626211.1998314043.19.JuwBlh这个链接进行申请 ！出发地数据可以根据类目属性接口来查看，链接：http://api.taobao.com/apidoc/api.htm?path=cid:20280-apiId:21802
	 **/
	...
	/** 
	 * 住宿晚数，小于或等于行程晚数，且必须大于0，这里注意下，trip_days（行程天数）>=accom_nights（行程晚数）>=hotel_days（住宿晚数）
	 **/
	private $hotelDays;
	...
	/** 
	 * 商品主图-图片空间，商品主图需要关联的图片空间的绝对url。这个url所对应的图片必须要属于当前用户。pic_path和image只需要传入一个,如果两个都传，默认选择pic_path
	 **/
	private $picPath;
	...
	/** 
	 * 退改规则 1）格式：标准规则 或 自定义规则：a_a_num,a_b_num,b-1_c_num,c-1_c-1_num 2）规则：自定义规则里最多可含5组规则.示例描述：出发前30天(不含)以上买家通知取消，不收取任何费用, 出发30日至15日，按订单费用总额5%，支付卖家业务损失费。 出发14日至7日，按订单费用总额15%，支付卖家业务损失费。 出发6日至4日，按订单费用总额70%，支付卖家业务损失费。 出发3日至1日，按订单费用总额85%，支付卖家业务损失费。 出发当日，按订单费用总额90%，支付卖家业务损失费。
	 **/
	private $refundRegulations;
	
	/** 
	 * 退改规则类型，0为标准，1为自定义 3为不支持退改 不传，默认为0
	 **/
	private $refundType;
	...
	/** 
	 * 商品售卖类型，0为普通商品，1为预约商品；默认为0 saletype为1的时候，bc_start_date为必填参数。设置为预约商品的宝贝不能再修改为普通商品
	 **/
	private $saletype;
	...
	/** 
	 * 行程天数，必须大于0 这里注意下，trip_days（行程天数）>=accom_nights（行程晚数）>=hotel_days（住宿晚数）
	 **/
	private $tripDays;
	...
	{
		return "taobao.travel.item.modify";
	}
